...
parser.add_argument('--k', type=int, default=30, help='Num of nearest neighbors to use')
...
parser.add_argument('--point', type= int, default=1024, help= '')
parser.add_argument('--output', help= 'Folder for output visualization')
parser.add_argument('--num_votes', type=int, default=10, help='aggregate segmentation scores with voting')
...
class Manager():
    ...
    def test(self, test_data):
        ...
        for i, (cat_name, obj_ids, points, labels, mask, onehot) in enumerate(test_data):
            ...
            vote_pool = torch.zeros(points.size()[0], points.size()[1], num_part).cuda()
